  ...
  const handleUpdateTeamMember = async (data) => {
    try {
      ...
      const response = await axios.put(
        `${ServerAPI}updateTeamMember/${memberId}`,
        formData,
        {
          headers: {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${ServerAPI}getTeamMemberToUpdate/${memberId}`
        );
        if (response.data.success) {
          const TeamMemberData = response.data.TeamData;
          setValue("employee", TeamMemberData.employee);
          setValue("designation", TeamMemberData.designation);
          setValue("photo", TeamMemberData.photo);
          setValue("about", TeamMemberData.about);

          if (TeamMemberData.photo) {
            setPhoto(TeamMemberData.photo);
            const imageUrl = TeamMemberData.photo;
            if (setPreviewImage) {
              setPreviewImage(imageUrl);
            }
          }
    ...
    <div className="p-4 w-full xs:ml-80">
      <div className="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700  flex justify-center">
        <div className="w-full max-w-4xl p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700">
          <form onSubmit={handleSubmit(handleUpdateTeamMember)}>
            <div className="mb-6">
