...
const Update_must_visit_page = () => {
  ...
  const handleUpdateMustVisit = async (data) => {
    try {
      ...
      const response = await axios.put(
        `${ServerAPI}updateMustVisit/${restaurantId}`,
        formData,
        {
          headers: {
  ...
  const validateRate = (value) => {
    ...
    if (isNaN(parsedValue) || parsedValue < 1 || parsedValue > 5) {
      return "Please enter a valid rate between 1 and 5";
    }
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${ServerAPI}getMustVisitToUpdate/${restaurantId}`
        );
        if (response.data.success) {
          ...
          setValue("rate", restaurantData.rate);
    ...
    <div className="p-4 w-full xs:ml-80">
      <div className="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700  flex justify-center">
        <div className="w-full max-w-4xl p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700">
          <form onSubmit={handleSubmit(handleUpdateMustVisit)}>
            <div className="mb-6">
              <h3 className="w-full text-center text-xl text-black my-5">
                Update Must visit Restaurant
              </h3>
              <label
                for="restaurant"
            ...
            <div className="mb-6">
              ...
              {errors.rate && errors.rate.type === "required" && (
                <label className="error-msg text-sm text-red-600">
                  Please enter the restaurant rate
                </label>
              )}
              {errors.rate && (
                <label className="error-msg text-sm text-red-600">
...
export default Update_must_visit_page;
