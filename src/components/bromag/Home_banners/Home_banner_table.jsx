...
const Home_banner_table = () => {
  ...
  useEffect(() => {
    const handleHomeBanners = async () => {
      try {
        const response = await axios.get(`${ServerAPI}homeBanners`, {
          params: {
            state: state,
            city: city,
          },
  ...
  const handleRestaurantDrop = async (bannerId) => {
    try {
      const response = await axios.post(`${ServerAPI}deleteHomeBanners`, {
        bannerId: bannerId,
      });
      if (response.data.success) {
        if (refresh === true) {
            ...
            <tbody>
              {HomeBanners.map((restaurant, i) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">{restaurant.link}</td>
