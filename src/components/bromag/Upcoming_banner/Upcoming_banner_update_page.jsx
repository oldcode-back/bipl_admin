...
const Upcoming_banner_update_page = () => {
  ...
  const handleUpdateUpcomingBanner = async (data) => {
    try {
      setLoading(true);
      ...
      const response = await axios.put(
        `${ServerAPI}updateUpcomingBanner/${bannerId}`,
        formData,
        {
          headers: {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${ServerAPI}getUpcomingBannerToUpdate/${bannerId}`
        );
        if (response.data.success) {
  ...
  return (
    <div className="p-4 w-full xs:ml-80">
      ...
      <div className="w-full overflow-x-auto h-[628px]  max-w-4xl p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700">
          <form onSubmit={handleSubmit(handleUpdateUpcomingBanner)}>
            <div className="mb-6">
              <h3 className="w-full text-center text-xl text-black my-5">
                Update Upcoming Banner
              </h3>
              <label
                for="banner"
...
export default Upcoming_banner_update_page;
